...
- **Codex CLI rollout parity** — Codex ingests now carry the same fidelity as Claude Code: per-step tokens with cache and reasoning splits, real per-turn model ids (from `turn_context`), per-turn latency, and structured file changes from `patch_apply_end`. Codex sessions also appear in the live fleet view (`meter web --live`) via tail-poll discovery of `~/.codex` rollouts.
...
- **Probe honesty** — the Live Probe panel and endpoints are gated to runtimes that actually implement pause/ack (the TS/Python SDKs); transcript- and proxy-sourced runs no longer render a pause button that would wedge them.
